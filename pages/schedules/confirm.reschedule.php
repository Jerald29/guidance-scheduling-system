...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["confirm_reschedule"])) {
    ...
    // Confirm start
    $query = "UPDATE tbl_schedules SET status = 'confirmed' WHERE schedule_id = ?";
    ...
    if ($stmt->execute()) {
        ...
        while ($adminRow = mysqli_fetch_assoc($adminQuery)) {
            ...
            $message = "Student: {$scheduleData['student_lname']}, {$scheduleData['student_fname']} has confirmed the rescheduled appointment on $formattedDate at $formattedTime.";
        ...
        $_SESSION['success'] = "Your rescheduled appointment has been confirmed!";
        header("Location: list.my.schedule.php?success=confirmed");
        exit();
    } else {
